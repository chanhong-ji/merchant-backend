...
import { ICreateUserInput } from 'src/modules/user/application/dto/create-account.dto';
import { UserRepository } from 'src/modules/user/application/user.repository';
import { CreateUserUsecase } from 'src/modules/user/domain/usecase/create-account.usecase';
...
describe('CreateAccountUsecase', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [CreateUserUsecase, { provide: 'UserRepository', useClass: TypeormUserRepository }, ErrorService],
    }).compile();
  ...
  describe('validateEmailDuplicate', () => {
    ...
    it('해당 이메일을 가진 유저가 이미 존재하면 -> 오류 발생', async () => {
      ...
      await expect(usecase.validateEmailDuplicate(input)).rejects.toThrow(errorService.get('EMAIL_ALREADY_EXIST'));
    });
  });
});
